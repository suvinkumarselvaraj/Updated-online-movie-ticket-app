  ...
  function handleSubmit(event) {
    ...
    if (data.get("username") == '' || data.get("password") == '')
      ...
      if (choice === 'signup') {
        ...
        fetch("http://localhost:8080/tickets/signup?id=" + uuidv4() + "&name=" + data.get("username") + "&password=" + data.get("password"), { crossDomain: true })
          .then((res) => res.json())
          .then(data => {
        ...
        //handling signin---------------------------------------------------------
          if(choice === 'signin'){
            fetch("http://localhost:8080/tickets/signin?name="+ data.get("username") + "&password=" + data.get("password"))
            .then((res) => res.json())
            .then(data=>{
                if(data.status === 'success'){
                  dispatch({
